...
<@layout.registrationLayout displayMessage=!messagesPerField.existsError('password','password-confirm'); section>
    ...
    <#elseif section = "form">
        <form action="${url.loginAction}" method="post" id="update-password-form">
            ...
            <div class="flex flex-none flex-col w-full" style="max-width: calc(384px);">
                <label for="update-password"
                       class="inline-block c-text-14 mb-0.5">${msg("passwordNew")}</label>
                <div class="flex flex-none">
                    <input id="update-password" name="password-new" type="password"
                           oninput="validateUpdatePasswordForm()"
                ...
                <span style="display: none" id="update-password-error-text-1"
                      class="block c-text-12 mt-1 text-error ml-4 ml-4">${msg("minimumLengthIsFiveCharacters")}</span>
                <span style="display: none" id="update-password-error-text-2"
                      class="block c-text-12 mt-1 text-error ml-4 ml-4">${msg("shouldContainANumber")}</span>
            </div>
            <div class="flex flex-none flex-col w-full mt-8" style="max-width: calc(384px);">
                <label for="update-password-repeat"
                       class="inline-block c-text-14 mb-0.5">${msg("passwordConfirm")}</label>
                <div class="flex flex-none">
                    <input autocomplete="new-password" name="password-confirm" type="password"
                           id="update-password-repeat" oninput="validateUpdatePasswordForm()"
                           class="w-full tracking-[unset] border border-solid border-mid-gray rounded-lg h-12 px-4 focus-visible:outline-none focus-visible-border-dark-blue focus-visible:border-2">
                </div>
                <span style="display: none" id="update-password-repeat-error-text"
                      class="block c-text-12 mt-1 text-error ml-4 ml-4">${msg("notMatchPasswordMessage")}</span>
            </div>
            ...
            <div class="flex flex-row items-center gap-8 mt-8 flex-wrap">
                ...
                <button
                    ...
                    <span class="c-text-14">${msg("updatePasswordTitle")}</span>
