...
import { fetchOrders, setOrdersCurrentFilter } from "../store/administratorManagementToolSlice";
...
import AdministratorOrderListItem from "../components/administrator-management-tool/AdministratorOrderListItem";

function AdministratorManagementToolOrders() {
  const { t } = useTranslation();
  const dispatch = useDispatch();

  const { orders, ordersTotalPages, ordersCurrentPage, ordersCurrentFilter } = useSelector(
      ...
      {!orders?.length ? (
        ...
        <div className={"flex flex-col"}>
          <div className="flex flex-col gap-y-3">
            {(orders || []).map((order) => (
              <AdministratorOrderListItem order={order} key={order.id}></AdministratorOrderListItem>
            ))}
          </div>
          {ordersTotalPages > 1 && (
            <CPagination
...
export default AdministratorManagementToolOrders;
